namespace :routes do
  ...
  desc 'Brief listing of controller methods with plus matches to controller actions in routes.rb'
  ...
  def find_app_controller_methods_matched_to_controller_methods_in_routes(pattern='')
    controllers_in_routes = find_routes_controller_actions(@pattern)
    find_app_controller_actions(@pattern).map! {|p| p << controllers_in_routes.find {|i| i[0] == p[0]}}
  end
